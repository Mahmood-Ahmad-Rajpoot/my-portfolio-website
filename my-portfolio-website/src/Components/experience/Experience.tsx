import React from "react";
import { Button } from "antd";
import { DownloadOutlined } from "@ant-design/icons";
import Image from "next/image";
import { myPic2 } from "../../assets";
import SkillSet from "./SkillSet";

const Experience = () => {
  return (
    <div className="bg-white w-full px-8 pt-[100px] flex md:flex-row flex-col gap-3  justify-around">
      <div className="md:w-1/2 shrink w-full mb-5 flex items-center justify-center  ">
        {" "}
        <Image
          className="rounded-full shrink-none "
          width={500}
          src={myPic2}
          alt="my-Profile-pic"
        />{" "}
      </div>
      <div className="md:text-left text-center md:w-1/2 w-full flex flex-col   ">
        <p className="text-[--color-primary] font-semibold text-[20px] tracking-[2px] leading-normal">
          ABOUT ME
        </p>

        <h1 className="text-[--color-primary] font-bold leading-[75px] text-[60px]">
          4+ Years Experience
        </h1>
        <h3 className="text-[--color-secondary] font-bold leading-[75px] text-[60px] mb-10">
          On Web Development
        </h3>
        <p className=" mb-12 w-full sm:text-justify text-center text-light text-[18px] text-[--color-secondary] leading-[25px] text-light">
          Hello there! I'm <b>Mahmood Ahmad</b>. I specialize in web design and
          development, and I'm deeply passionate and committed to my craft. With
         <b> 4+ years</b> of experience as a professional graphic designer
        </p>
        <div className="flex md:flex-row flex-col w-full justify-left mt-5  ">
           
          <SkillSet />
        </div>
      </div>
    </div>
  );
};

export default Experience;
